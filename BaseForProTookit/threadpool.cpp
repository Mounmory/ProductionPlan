...
void ThreadPool::setTaskQueMaxThrshHold(const uint32_t& threshhold)
{
	IS_TRUE_RETURN(m_isPoolRunning)
	m_taskqueMaxThresHold = threshhold;
}

void ThreadPool::setThreadSizeThreshHold(const uint16_t& threshHold)
{
	IS_TRUE_RETURN(m_isPoolRunning)
...
void ThreadPool::setThreadSize(const uint16_t& treadSize)
{
	IS_TRUE_RETURN(m_isPoolRunning)
	if (treadSize < 0) 
	{
